...
export default function register(api) {
    ...
    a.registerTool({
        ...
        description: `Start the WSInsight Docker container (${DOCKER_IMAGE}) ` +
        ...
        parameters: Type.Object({
            data_dir: Type.String({
                description: "Absolute host path mounted as /workspace inside the container. Required.",
            }),
            gpu_ids: Type.Optional(Type.String({
                description: "Comma-separated GPU IDs to expose (e.g. '0' or '0,1'). Default: all.",
            })),
            mcp_port: Type.Optional(Type.Number({
            ...
            experimental: Type.Optional(Type.Boolean({
                description: "Enable experimental tools (hplot, ecomp, tcomp, cme). Default: false.",
            })),
        }),
        async execute(_id, params) {
            ...
            if (!dataDir) {
                return WsInsightMcpClient.asText("wsinsight_start_docker", "[ERROR] 'data_dir' is required");
            }
            ...
            const cn = typeof params.container_name === "string"
            ...
            try {
                ...
                return WsInsightMcpClient.asText("wsinsight_start_docker", `[ERROR] docker run failed: ${err}`);
            }
        },
    }, OPT);
    ...
    a.registerTool({
        ...
        description: "Stop and remove the WSInsight MCP Docker container. " +
            "All running jobs inside will be terminated.",
        parameters: Type.Object({
            container_name: Type.Optional(Type.String({ description: "Container name to stop. Default: clawsight-mcp." })),
        }),
        async execute(_id, params) {
            ...
            try {
                ...
                await WsInsightMcpClient.dockerRunIgnoreError(["rm", cn]);
                client.reset();
                return WsInsightMcpClient.asText("wsinsight_stop_docker", `Container '${cn}' stopped and removed.`);
            }
            catch (err) {
                return WsInsightMcpClient.asText("wsinsight_stop_docker", `[WARN] ${err}`);
            }
        },
    }, OPT);
