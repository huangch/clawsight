...
import { ENGINES, state, type Engine, type EngineState, type ToolEntry } from "./engines.js";
...
const posixIds = process as unknown as {
  getuid?: () => number;
  getgid?: () => number;
};
...
): { ok: true; value: Record<string, unknown> } | { ok: false; err: string } {
...
async function refreshTools(st: EngineState): Promise<string | null> {
  try {
    const client = st.client();
    const tools = (await client.listTools()) as unknown as ToolEntry[];
    st.tools = tools;
    return null;
  } catch (e) {
    st.reset();
...
function formatTools(engine: Engine, tools: ToolEntry[]): string {
  if (!tools.length) return "(no tools reported)";
...
function makeStart(engine: Engine) {
  return async (params: Record<string, unknown>): Promise<string> => {
    ...
    if (!Number.isFinite(port)) return err("'port' must be an integer");
    const cname =
      String(params["container_name"] ?? st.container).trim() || st.container;
    ...
    if (experimental && engine.experimental) cmd += " --experimental";
    ...
    if (toolsErr) {
      lines.push(
        `  tools   : not discovered yet (${toolsErr.split("\n")[0]?.slice(0, 80) ?? toolsErr}); ` +
...
function makeStop(engine: Engine) {
  return async (params: Record<string, unknown>): Promise<string> => {
    const st = state(engine.name);
    const cname =
      String(params["container_name"] ?? st.container).trim() || st.container;
...
function makeStatus(engine: Engine) {
  return async (_params: Record<string, unknown>): Promise<string> => {
    const st = state(engine.name);
    ...
    if (refreshErr) {
      lines.push(
        `tools     : unreachable (${refreshErr.split("\n")[0]?.slice(0, 80) ?? refreshErr})`,
      );
    } else {
...
function makeListTools(engine: Engine) {
  return async (params: Record<string, unknown>): Promise<string> => {
    const st = state(engine.name);
    ...
    if (refreshErr !== null) {
      return err(
        `Cannot reach the ${engine.name} MCP server at ${st.mcpUrl}.\n` +
          `Start it with ${engine.name}_start(data_dir=...).\n` +
          `Detail: ${refreshErr}`,
      );
    }
...
function makeCall(engine: Engine) {
  return async (params: Record<string, unknown>): Promise<string> => {
    const st = state(engine.name);
    const tool = String(params["tool"] ?? "").trim();
    if (!tool) {
      return err(
        `'tool' is required. Run ${engine.name}_list_tools() to see what ` +
          `this engine exposes.`,
      );
    }
...
type Verb = keyof typeof FACTORIES;
const VERBS: Verb[] = ["start", "stop", "status", "list_tools", "call"];

export type Handler = (params: Record<string, unknown>) => Promise<string>;
