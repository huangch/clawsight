...
function startSchema(e: Engine): JsonSchema {
  ...
  if (e.gpu) {
    properties.gpu_ids = {
      type: "string",
      description:
        'Comma-separated GPU ids to expose, e.g. "0,1". Empty or ' +
  ...
  if (e.experimental) {
    properties.experimental = {
      type: "boolean",
      description:
        "Expose experimental sub-commands as tools " +
  ...
  return {
    type: "object",
    description:
      `Start the ${e.name} Docker container and its MCP server, then ` +
...
function stopSchema(e: Engine): JsonSchema {
  return {
    type: "object",
...
function statusSchema(e: Engine): JsonSchema {
  return {
    type: "object",
    description:
      `Report whether the ${e.name} container is running, its MCP URL, ` +
...
function listToolsSchema(e: Engine): JsonSchema {
  return {
    type: "object",
    description:
      `List the tools the running ${e.name} server exposes, with a short ` +
    ...
    properties: {
      tool: {
        type: "string",
        description:
          "Return the full JSON input schema for this one tool instead " +
          "of the summary list.",
      },
    },
    required: [],
...
function callSchema(e: Engine): JsonSchema {
  return {
    type: "object",
    description:
      `Invoke any tool exposed by the running ${e.name} server. Use ` +
    ...
    properties: {
      ...
      arguments: {
        type: "object",
        description:
          "Arguments object for that tool, matching its input schema.",
      },
    },
    required: ["tool"],
  };
...
const TEMPLATES: Record<string, (e: Engine) => JsonSchema> = {
  start: startSchema,
  stop: stopSchema,
  status: statusSchema,
...
const VERBS = Object.keys(TEMPLATES) as Array<keyof typeof TEMPLATES>;
...
export function jsonSchemaToTypeBox(schema: JsonSchema): TSchema {
  ...
  for (const key of requiredKeys) {
    if (props[key] === undefined) props[key] = Type.String();
  }
  if (Object.keys(props).length === 0) return Type.Object({});
 ...
 */
export function buildTools(): Record<string, ToolDefinition> {
