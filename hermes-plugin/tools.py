...
class _State:
    def __init__(self) -> None:
        self.mcp_url: str = os.environ.get("WSINSIGHT_MCP_URL", _DEFAULT_MCP_URL)
        self.timeout_s: float = (
            int(os.environ.get("WSINSIGHT_MCP_TIMEOUT_MS", "300000")) / 1000.0
        )
        self.container: str = os.environ.get("WSINSIGHT_CONTAINER_NAME", _DEFAULT_CNAME)
...
async def wsinsight_job_logs(params: dict, **_: Any) -> str:
    ...
    if (tail := params.get("tail")) is not None:
        args["tail"] = int(tail)
    try:
